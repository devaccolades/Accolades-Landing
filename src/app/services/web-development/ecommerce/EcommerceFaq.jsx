...
export default function EcommerceFaq({ faqData, title, subtitle }) {
  ...
  return (
    <section className="containers  font-mont mx-auto py-12">
      ...
      {subtitle && (
        <p className="text-[13px] md:text-[15px] text-gray-600 max-w-3xl mb-10">
          {subtitle}
        </p>
      )}
      <div className="space-y-0 border-t border-gray-300">
        {faqData.map((item, i) => (
          <div key={i} className="border-b border-gray-300">
            ...
            >
              <p className="text-black font-semibold font-mont text-[13px] md:text-[14px] leading-relaxed px-1">
                {item.a}
              </p>
            </div>
          </div>
